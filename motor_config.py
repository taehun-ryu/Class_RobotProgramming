""" 
모션제어 GUI의 배포버전. test_config_ver.py파일이 테스트 성공 시 새롭게 업데이트 할 것.
Author: Ryu Taehun
"""
...
bundle = modi.MODI()
motor1=bundle.motors[0] # 제일 앞에 있는 모터를의미
motor2=bundle.motors[1]
motor3=bundle.motors[2]
motor4=bundle.motors[3]
motor5=bundle.motors[4]
motor6=bundle.motors[5]
...
label_set1=tk.Label(window, text="이 줄이 로봇 기준 Right")
label_set1.grid(row=0, column=0)
label_set2=tk.Label(window, text="이 줄이 로봇 기준 Left")
...
def btnCmd(index):
    ...
    if index <=5:
        ...
        if index %2 ==0:
            bundle.motors[flr_index_2].first_degree = m_degree
        else:
            bundle.motors[flr_index_2].second_degree = m_degree
    else:
...
def btnInitial():
    ...
    for i,j in zip([0,1,2,3,4,5],[0,2,4,6,8,10]):
            bundle.motors[i].first_degree = initial_pose[j]
            bundle.motors[i].second_degree = initial_pose[j+1]
...
def btnleft():
    ...
    for i in [0,1,2]:
            bundle.motors[i].first_degree = left_triangle_pose[2*i]
            bundle.motors[i].second_degree = left_triangle_pose[2*i+1]
    
    print("Move right Motors:",left_triangle_pose)
...
def btnright():
    ...
    for i in [3,4,5]:
            bundle.motors[i].first_degree = right_triangle_pose[2*(i-3)]
            bundle.motors[i].second_degree = right_triangle_pose[2*(i-3)+1] 

    print("Move left Motors:",right_triangle_pose)
...
btn_left = tk.Button(window, text="right모터만 실행", command=btnleft)
btn_left.grid(row=3,column=6)

btn_right = tk.Button(window, text="left모터만 실행", command=btnright)
...
btn_stack_start = tk.Button(window, text="동작 배열 실행",command=startStack)
btn_stack_start.grid(row=11,column=6)

move_label_start = tk.Label(window, text="-----------모터 12개 동작 배열(R L 순). 최대"+str(range_move)+"개----------")
